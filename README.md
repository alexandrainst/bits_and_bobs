...
In the `src` directory there are two subdirectories, `bits_and_bobs`
and `scripts`. This is a brief explanation of the differences between the two.

### Modules

All Python files in the `bits_and_bobs` directory are _modules_
...
```python
from bits_and_bobs import some_function
```
...
`alexandrainst/bits_and_bobs` then simply press the `<> Code` button
